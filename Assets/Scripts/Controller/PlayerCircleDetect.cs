...
public class PlayerCircleDetect : MonoBehaviour {
    ...
    public void SetButtonShow (bool flag) {
        if (curTarget != null) {
            ...
            if (flag) {
                //只有在非机械臂状态下才能交互
                //Debug.Log ("breakType:" + transform.parent.GetComponentInChildren<PlayerChildController> ().attackType.ToString());
                if (showCoroutine == null && transform.parent.GetComponentInChildren<PlayerChildController> ().attackType == BreakLevel.easy) {
